...
export class CartsManagerMongo {
  ...
  async getCarts () {
    try {
      const carts = await this.model.find().populate('products.productId').lean();
  ...
  async updateProductQuantity (cartId, productId, newQuantity) {
    try {
      ...
      if (productIndex === -1) throw new Error('No se puede actualizar la cantidad porque el producto no se agregó al carrito');

      // luego verifico si el dato ingresado es de tipo numérico
      if (typeof newQuantity !== 'number') throw new Error('La cantidad ingresada debe ser de tipo numérico.');
  ...
  async deleteProduct (cartId, productId) {
    try {
      ...
      const productExist = cart.products.some((prod) => prod.productId._id.toString() === productId);
      if (!productExist) throw new Error('El producto que quiere eliminar no se encuentra en el carrito.');

      const newProducts = cart.products.filter((prod) => prod.productId._id.toString() !== productId);
